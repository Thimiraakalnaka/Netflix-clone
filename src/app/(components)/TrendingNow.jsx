...
function TrendingNow() {
    ...
    return (
        <div className='mb-16'>
            <h1 className='text-[1.125rem] md:text-[1.5rem] font-[700] mb-4'>TrendingNow</h1>
            <div className='relative flex justify-center items-center'>
                ...
                <div id='slider' className='w-full h-full py-2 overflow-x-scroll scroll whitespace-nowrap scroll-smooth scrollbar-hide'>
                    {films.map((film) => {
                        return (
                            <div key={film.id} className='inline-block rounded-xl cursor-pointer px-2 mx-3 md:mx-5 h-[156px] w-[112px] md:h-[252px] md:w-[180px] hover:scale-105 transition-transform duration-250 ease-in-out bg-center bg-cover' style={{ backgroundImage: `url(${film.url1})` }} onClick={()=>handleclick(film)}>
                                <div className='mt-14 md:mt-[8rem] ml-[-1rem] md:ml-[-1rem] '>
                                    <h1 className='text-black text-[4rem] md:text-[5rem] font-extrabold text-stroke cursor-pointer'>{film.id}</h1>
                                </div>
                            </div>
                        )
                    })}
